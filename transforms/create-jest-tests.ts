...
export default function transform(file, api) {
  const codeApi: JSCodeshift = api.jscodeshift
  const ast = codeApi(file.source);
  const filename = path.basename(file.path);
  const description = `${filename} tests`;

  // setup required libs
  const requiredLibs = getRequiredLibs(codeApi, ast).map((id) => {
  ...
  // setup tests
  const tests = getTestInfo(codeApi, ast)
    .sort(sortByName)
    .map((testInfo) => {
